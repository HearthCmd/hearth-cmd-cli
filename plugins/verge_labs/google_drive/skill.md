...
The connection slug is shown in your resource list (e.g. `flogg_drive`).
...
```
hearth resource flogg_drive search_files --arg query="name contains 'budget'"
hearth resource flogg_drive search_files --arg query="fullText contains 'Q3 revenue'"
hearth resource flogg_drive search_files --arg query="mimeType = 'text/plain'"
```
...
```
hearth resource flogg_drive list_folder_contents --arg folder_id=1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgVE2upms
```
...
```
hearth resource flogg_drive download_file --arg file_id=<id>
```
...
hearth resource flogg_drive export_file --arg file_id=<id> --arg mime_type=text/plain

# Google Doc → Word
hearth resource flogg_drive export_file --arg file_id=<id> --arg mime_type=application/vnd.openxmlformats-officedocument.wordprocessingml.document

# Google Sheet → CSV
hearth resource flogg_drive export_file --arg file_id=<id> --arg mime_type=text/csv

# Any file → PDF
hearth resource flogg_drive export_file --arg file_id=<id> --arg mime_type=application/pdf
```
...
```
hearth resource flogg_drive get_file_metadata --arg file_id=<id>
```
...
hearth resource flogg_drive create_file \
  --arg name="notes.md" \
...
```
hearth resource flogg_drive upload_file_content \
  --arg file_id=<id> \
  --arg mime_type=text/markdown \
...
Rename:
```
hearth resource flogg_drive rename_file --arg file_id=<id> --arg name="New name.md"
```

Move (requires the current parent ID — get it from `get_file_metadata`):
```
hearth resource flogg_drive move_file \
  --arg file_id=<id> \
...
Create a folder:
```
hearth resource flogg_drive create_folder \
...
```
hearth resource flogg_drive trash_file --arg file_id=<id>
```
...
hearth resource flogg_drive share_file \
  --arg file_id=<id> \
  --arg role=writer \
  --arg type=user \
