...
func buildDisplayPrompt(displaysJSON string) string {
	...
	if len(publishable) > 0 {
		...
		b.WriteString("You can also scroll a long page you've already put up, without republishing — ")
		b.WriteString("useful hands-free, when someone speaking to you says \"scroll down\" or \"back to the top\":\n\n")
		b.WriteString("  hearth display scroll down --target <id>     # one page (add --amount half for less)\n")
		b.WriteString("  hearth display scroll top --target <id>      # or: up | down | bottom\n\n")
		b.WriteString("Scroll works on markdown and inline html content; a plain `--type url` page can't be ")
		b.WriteString("scrolled (it's isolated), so publish long content as markdown or html if it needs to scroll.\n\n")
